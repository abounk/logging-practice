...
def logging_test(logger):
    ...
    """
    # debug
    logging.debug("results = {'msg': 'hello'}")
    # info
    logging.info("Tim has logged in")
    # warning
    logging.warning("Someone trying to login with id=Tim")
    # level = logging.WARN + 5 (custom log level between WARN and ERROR)
    level = logging.WARN + 5  # custom log level
    logger.log(level, "This will cause an error")
    # error
    logging.error("expected byte-like object not int()")
    # critical or fatal
    logging.critical("No module name 'Assignment'")
...
if __name__ == "__main__":
    ...
    mylogger = logging.getLogger("loveISP")
